...
const visitorSchema = new mongoose.Schema({ path: { type: String, default: '/', maxlength: 200 }, referrer: { type: String, default: 'direct', maxlength: 200 }, device: { type: String, default: 'desktop', maxlength: 20 }, country: { type: String, default: 'unknown', maxlength: 80 }, sessionHash: String, createdAt: { type: Date, default: Date.now } });
...
async function sendVerificationEmail(user, token) {
  ...
  return safeEmail({ to: user.email, subject: 'Verify your Lee Tech account', text: `Verify your account: ${link}`, html: `<p>Welcome to Lee Tech, ${escapeHtml(user.username)}.</p><p><a href="${escapeHtml(link)}">Verify your email address</a></p><p>This link expires in 24 hours.</p>` });
}
async function sendResetEmail(user, token) {
  const link = `${APP_URL}/?reset=${encodeURIComponent(token)}&username=${encodeURIComponent(user.username)}`;
  return safeEmail({ to: user.email, subject: 'Reset your Lee Tech password', text: `Reset your password: ${link}`, html: `<p>A password reset was requested for ${escapeHtml(user.username)}.</p><p><a href="${escapeHtml(link)}">Reset your password</a></p><p>This link expires in one hour.</p>` });
}
...
app.post('/api/analytics/visit', requireDatabase, async (req, res) => { try { await Visitor.create({ path: clean(req.body.path || '/', 200).startsWith('/') ? clean(req.body.path || '/', 200) : '/', referrer: clean(req.body.referrer || 'direct', 200), device: deviceFromAgent(req.headers['user-agent']), sessionHash: sessionHash(req) }); res.status(204).end(); } catch { res.status(204).end(); } });
...
app.post('/api/auth/user/register', requireDatabase, loginLimiter, async (req, res) => { try { const username = normalizeUsername(req.body.username); const email = normalizeEmail(req.body.email); const password = String(req.body.password || ''); if (!validUsername(username)) return res.status(400).json({ error: 'Choose a username with 3–30 lowercase letters, numbers, or hyphens. That username is reserved or invalid.' }); if (!isEmail(email)) return res.status(400).json({ error: 'Enter a valid email address' }); if (password.length < 12) return res.status(400).json({ error: 'Password must be at least 12 characters' }); if (await User.exists({ $or: [{ username }, { email }] })) return res.status(409).json({ error: 'That username or email is already registered' }); const passwordData = hashPassword(password); const verificationToken = randomToken(); const user = await User.create({ username, email, passwordHash: passwordData.hash, passwordSalt: passwordData.salt, emailVerificationTokenHash: hashToken(verificationToken), emailVerificationExpiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000), displayName: username }); await sendVerificationEmail(user, verificationToken); recordAudit(req, 'user_register', true, username); res.status(201).json({ message: 'Account created. Check your email to verify your account.', user: publicUser(user) }); } catch (error) { if (error?.code === 11000) return res.status(409).json({ error: 'That username or email is already registered' }); res.status(400).json({ error: 'Unable to create account' }); } });
app.get('/api/auth/user/verify-email', requireDatabase, async (req, res) => { try { const token = clean(req.query.token, 200); const username = normalizeUsername(req.query.username); const user = await User.findOne({ username, emailVerificationTokenHash: hashToken(token), emailVerificationExpiresAt: { $gt: new Date() } }); if (!user) return res.status(400).json({ error: 'Verification link is invalid or expired' }); user.emailVerified = true; user.emailVerificationTokenHash = ''; user.emailVerificationExpiresAt = null; user.updatedAt = new Date(); await user.save(); await safeEmail({ to: user.email, subject: 'Your Lee Tech account is verified', text: `Your account ${user.username} is ready.`, html: `<p>Your Lee Tech account is verified. Your site is <a href="${escapeHtml(`${PUBLIC_SITE_BASE_URL}/${user.username}`)}">${escapeHtml(`${PUBLIC_SITE_BASE_URL}/${user.username}`)}</a>.</p>` }); res.json({ message: 'Email verified successfully', user: publicUser(user) }); } catch { res.status(400).json({ error: 'Unable to verify account' }); } });
...
app.post('/api/auth/user/reset-password', requireDatabase, async (req, res) => { try { const username = normalizeUsername(req.body.username); const token = clean(req.body.token, 200); const password = String(req.body.password || ''); if (password.length < 12) return res.status(400).json({ error: 'Password must be at least 12 characters' }); const user = await User.findOne({ username, passwordResetTokenHash: hashToken(token), passwordResetExpiresAt: { $gt: new Date() } }); if (!user) return res.status(400).json({ error: 'Reset link is invalid or expired' }); const passwordData = hashPassword(password); user.passwordHash = passwordData.hash; user.passwordSalt = passwordData.salt; user.passwordResetTokenHash = ''; user.passwordResetExpiresAt = null; user.updatedAt = new Date(); await user.save(); res.json({ message: 'Password reset successfully' }); } catch { res.status(400).json({ error: 'Unable to reset password' }); } });

app.use('/api/me', requireDatabase);
app.get('/api/me', userAuth, (req, res) => res.json({ user: publicUser(req.user) }));
app.put('/api/me/profile', userAuth, verifiedUser, async (req, res) => { req.user.displayName = clean(req.body.displayName || req.user.username, 120); req.user.bio = clean(req.body.bio || '', 600); req.user.updatedAt = new Date(); await req.user.save(); res.json({ user: publicUser(req.user) }); });
...
app.get('/api/wallet', requireDatabase, userAuth, verifiedUser, async (req, res) => { const transactions = await WalletTransaction.find({ userId: req.user._id }).sort({ createdAt: -1 }).limit(100).lean(); const payments = await PaymentTransaction.find({ userId: req.user._id }).sort({ createdAt: -1 }).limit(50).lean(); res.json({ balanceMinor: req.user.walletBalanceMinor, transactions, payments, currency: PAYSTACK_CURRENCY }); });
