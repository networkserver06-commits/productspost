...
const productSchema = new mongoose.Schema({ name: { type: String, required: true, trim: true, maxlength: 120 }, description: { type: String, required: true, trim: true, maxlength: 4000 }, price: { type: Number, required: true, min: 0 }, productType: { type: String, enum: ['digital', 'physical'], default: 'digital' }, stock: { type: Number, min: 0, default: null }, category: { type: String, default: 'General', trim: true, maxlength: 80 }, image: { type: String, default: '', maxlength: 2000000 }, featured: { type: Boolean, default: false }, createdAt: { type: Date, default: Date.now }, expiresAt: { type: Date, default: null } });
...
function normalizePostFields(body = {}, current = {}) {
  ...
  return { title, content, excerpt, image: normalizeImage(body.image ?? current.image), contentType, productType, priceMinor, stock: normalizeStock(body.stock ?? current.stock, productType), published: body.published === true };
}
...
app.post('/api/me/posts', requireDatabase, userAuth, verifiedUser, async (req, res) => { const normalized = normalizePostFields(req.body); if (!normalized.title || !normalized.content) return res.status(400).json({ error: 'Title and content are required' }); const session = await mongoose.startSession(); try { let post; let charge = 0; await session.withTransaction(async () => { const chargeResult = normalized.published ? await chargeForPost(req.user._id, normalized.title, session) : { priceMinor: 0 }; charge = chargeResult.priceMinor; const created = await Post.create([{ ...normalized, author: req.user.displayName || req.user.username, ownerId: req.user._id, ownerUsername: req.user.username, updatedAt: new Date() }], { session }); post = created[0]; }); res.status(201).json({ post, chargedMinor: charge }); } catch (error) { if (error.code === 'INSUFFICIENT_BALANCE') return res.status(402).json({ error: 'Insufficient wallet balance. Top up before publishing.' }); res.status(400).json({ error: error.message || 'Unable to save post' }); } finally { await session.endSession(); } });
app.put('/api/me/posts/:id', requireDatabase, userAuth, verifiedUser, async (req, res) => { const session = await mongoose.startSession(); try { let post; let charge = 0; await session.withTransaction(async () => { const current = await Post.findOne({ _id: req.params.id, ownerId: req.user._id }).session(session); if (!current) { const error = new Error('NOT_FOUND'); error.code = 'NOT_FOUND'; throw error; } const normalized = normalizePostFields(req.body, current); if (!normalized.title || !normalized.content) { const error = new Error('INVALID'); error.code = 'INVALID'; throw error; } if (normalized.published && !current.published) charge = (await chargeForPost(req.user._id, normalized.title, session)).priceMinor; Object.assign(current, normalized, { author: req.user.displayName || req.user.username, updatedAt: new Date() }); await current.save({ session }); post = current; }); res.json({ post, chargedMinor: charge }); } catch (error) { if (error.code === 'NOT_FOUND') return res.status(404).json({ error: 'Post not found' }); if (error.code === 'INSUFFICIENT_BALANCE') return res.status(402).json({ error: 'Insufficient wallet balance. Top up before publishing.' }); res.status(400).json({ error: error.message || 'Unable to update post' }); } finally { await session.endSession(); } });
