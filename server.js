...
app.put('/api/admin/settings', async (req, res) => { const postPriceMinor = amountToMinor(req.body.postPrice, true); if (postPriceMinor === null) return res.status(400).json({ error: 'Enter a valid post price' }); const update = { key: 'main', postPriceMinor, siteMaintenance: req.body.siteMaintenance === true, siteMaintenanceMessage: clean(req.body.siteMaintenanceMessage || 'Lee Tech is temporarily unavailable while we make improvements.', 240), paymentsMaintenance: req.body.paymentsMaintenance === true, paymentsMaintenanceMessage: clean(req.body.paymentsMaintenanceMessage || 'Payments are temporarily unavailable. Please try again later.', 240), signupsRestricted: req.body.signupsRestricted === true, signupRestrictionMessage: clean(req.body.signupRestrictionMessage || 'New account registration is temporarily paused. Please try again later.', 240), updatedBy: req.admin.username, updatedAt: new Date() }; const settings = await AppSettings.findOneAndUpdate({ key: 'main' }, update, { upsert: true, new: true, runValidators: true }); res.json(settings); });
