...
app.get('/api/admin/stats', authMiddleware, async (req, res) => { try { res.json({ products: await Product.countDocuments(), posts: await Post.countDocuments(), featured: await Product.countDocuments({ featured: true }), visitors: await Visitor.countDocuments(), notes: await AdminNote.countDocuments() }); } catch { res.status(500).json({ error: 'Server error' }); } });
