...
app.post('/api/me/posts', requireDatabase, userAuth, verifiedUser, async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const created = await Post.create([{ ...normalized, moderationStatus: 'pending', author: req.user.displayName || req.user.username, ownerId: req.user._id, ownerUsername: req.user.username, updatedAt: new Date() }], { session });
      post = created[0];
    });
...
app.get('/api/admin/moderation', async (req, res) => {
  try {
    const [posts, products] = await Promise.all([
      Post.find().sort({ updatedAt: -1, createdAt: -1 }).limit(300).populate('ownerId', 'username email').lean(),
      Product.find().sort({ createdAt: -1 }).limit(300).lean()
    ]);
    const rows = [
      ...posts.map(item => ({ ...item, itemType: 'post', displayTitle: item.title, owner: item.ownerId?.username || 'Lee Tech' })),
      ...products.map(item => ({ ...item, itemType: 'product', displayTitle: item.name, owner: 'Lee Tech' }))
...
app.put('/api/admin/moderation/:type/:id', async (req, res) => {
  ...
  try {
    ...
    const item = await Model.findByIdAndUpdate(req.params.id, { moderationStatus: action === 'approve' ? 'approved' : 'rejected', moderationNote: clean(req.body.note || '', 500), moderatedAt: new Date(), moderatedBy: req.admin.username, ...(type === 'post' || type === 'product' ? { published: action === 'approve' } : {}) }, { new: true, runValidators: true }).lean();
