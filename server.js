...
async function chargeForPost(userId, postTitle, session, expectedPriceMinor = null) {
...
app.post('/api/me/posts', requireDatabase, userAuth, verifiedUser, async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const chargeResult = normalized.published ? await chargeForPost(req.user._id, normalized.title, session, expectedPriceMinor) : { priceMinor: 0, balanceAfterMinor: null };
      charge = chargeResult.priceMinor;
      balanceAfterMinor = chargeResult.balanceAfterMinor;
      const created = await Post.create([{ ...normalized, author: req.user.displayName || req.user.username, ownerId: req.user._id, ownerUsername: req.user.username, updatedAt: new Date() }], { session });
      post = created[0];
    });
