...
const heroImagePath = path.join(process.cwd(), 'public', 'lee-tech-hero.webp');
...
const heroImage = fs.existsSync(heroImagePath) ? fs.readFileSync(heroImagePath) : null;
...
function shareCardSvg({ title = 'Lee Tech', subtitle = 'Technology with intention.', kicker = 'LEE TECH COMMUNITY', url = 'POST.LEETEC.ONLINE', type = 'CREATOR SITE' } = {}) {
  const titleLines = wrapShareText(clean(title || 'Lee Tech', 90), 28).map(escapeHtml);
  const subtitleLines = wrapShareText(clean(subtitle || 'Technology with intention.', 170), 54).map(escapeHtml);
  const safeKicker = escapeHtml(clean(kicker || 'LEE TECH COMMUNITY', 42));
  const safeUrl = escapeHtml(clean(url || 'POST.LEETEC.ONLINE', 64));
  const safeType = escapeHtml(clean(type || 'CREATOR SITE', 24));
  const titleSvg = titleLines.map((line, index) => `<text x="120" y="300" dy="${index * 54}" fill="#10203a" font-family="DejaVu Sans" font-size="44" font-weight="700">${line}</text>`).join('');
  const subtitleSvg = subtitleLines.map((line, index) => `<text x="120" y="414" dy="${index * 30}" fill="#60708a" font-family="DejaVu Sans" font-size="21">${line}</text>`).join('');
  return `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#08152e"/><stop offset=".58" stop-color="#173f91"/><stop offset="1" stop-color="#2b67df"/></linearGradient><linearGradient id="accent" x1="0" y1="0" x2="1" y2="0"><stop stop-color="#bdebdc"/><stop offset="1" stop-color="#f1b247"/></linearGradient><filter id="shadow" x="-20%" y="-20%" width="140%" height="150%"><feDropShadow dx="0" dy="18" stdDeviation="18" flood-color="#071126" flood-opacity=".28"/></filter></defs><rect width="1200" height="630" fill="#eef2f7"/><rect x="26" y="26" width="1148" height="578" rx="42" fill="url(#bg)"/><path d="M700 26h474v245c-125 51-232 35-329-32-91-63-122-133-145-213Z" fill="#5c91ff" opacity=".28"/><circle cx="1035" cy="90" r="188" fill="#82aaff" opacity=".16"/><circle cx="80" cy="620" r="230" fill="#57c7ae" opacity=".22"/><path d="M26 470c150-78 264-66 381 32 52 44 100 78 150 102H26Z" fill="#bdebdc" opacity=".13"/><g font-family="DejaVu Sans"><rect x="86" y="78" width="68" height="68" rx="21" fill="#bdebdc"/><text x="120" y="123" text-anchor="middle" fill="#0b1830" font-size="37" font-weight="700">L</text><text x="180" y="101" fill="#bdebdc" font-size="15" font-weight="700" letter-spacing="3">${safeKicker}</text><text x="180" y="136" fill="#ffffff" font-size="27" font-weight="700">Lee Tech</text><rect x="86" y="182" width="1028" height="342" rx="34" fill="#ffffff" filter="url(#shadow)"/><rect x="120" y="226" width="88" height="7" rx="3" fill="url(#accent)"/><text x="120" y="265" fill="#1f5eff" font-size="13" font-weight="700" letter-spacing="2">${safeType}</text>${titleSvg}${subtitleSvg}<rect x="120" y="486" width="250" height="34" rx="17" fill="#eef3ff"/><text x="245" y="508" text-anchor="middle" fill="#1f5eff" font-size="13" font-weight="700">Explore the full story ↗</text><text x="1080" y="508" text-anchor="end" fill="#7d8798" font-size="16">${safeUrl}</text></g></svg>`;
}
...
function shareMetadata(req) {
  ...
  const isProductShare = mongoose.Types.ObjectId.isValid(shareProductId);
  const imageUrl = isProductShare ? new URL('/share-product-image.png', APP_URL) : (shareTitle || username ? new URL('/share-card.png', APP_URL) : new URL('/homepage-share-image.png', APP_URL));
  if (isProductShare) imageUrl.searchParams.set('product', shareProductId);
  if (!isProductShare && (shareTitle || username)) { imageUrl.searchParams.set('title', shareTitle || `@${username}`); imageUrl.searchParams.set('subtitle', shareText || `Published work and products from @${username} on Lee Tech.`); imageUrl.searchParams.set('kicker', username ? 'LEE TECH CREATOR SITE' : 'LEE TECH COMMUNITY'); imageUrl.searchParams.set('url', pageUrl.replace(/^https?:\/\//, '').split('?')[0]); imageUrl.searchParams.set('type', username ? 'CREATOR SITE' : 'LEE TECH SHARE'); }
  const imageMeta = isProductShare ? '' : '<meta property="og:image:type" content="image/png"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630">';
...
app.get('/api/products', requireDatabase, async (req, res) => { try { const products = await Product.find({ published: { $ne: false }, moderationStatus: { $nin: ['pending', 'rejected'] } }).sort({ featured: -1, createdAt: -1 }).limit(24).lean(); res.set('Cache-Control', 'no-store, must-revalidate'); res.json(products); } catch { res.status(500).json({ error: 'Server error' }); } });
app.get('/api/posts', requireDatabase, async (req, res) => { try { const posts = await Post.find({ published: { $ne: false }, ownerId: null, moderationStatus: { $nin: ['pending', 'rejected'] } }).sort({ createdAt: -1 }).limit(12).lean(); res.set('Cache-Control', 'no-store, must-revalidate'); res.json(posts); } catch { res.status(500).json({ error: 'Server error' }); } });
...
app.post('/api/me/posts', requireDatabase, userAuth, verifiedUser, async (req, res) => {
  const clientSubmissionId = clean(req.get('Idempotency-Key') || req.body.clientSubmissionId, 120);
  if (clientSubmissionId && !/^post-[A-Za-z0-9-]+$/.test(clientSubmissionId)) return res.status(400).json({ error: 'Invalid submission key' });
  const session = await mongoose.startSession();
  try {
    ...
    if (clientSubmissionId) {
      const existing = await Post.findOne({ ownerId: req.user._id, clientSubmissionId }).lean();
      if (existing) {
        const latest = await User.findById(req.user._id).select('walletBalanceMinor').lean();
        return res.status(200).json({ post: existing, chargedMinor: 0, balanceAfterMinor: latest?.walletBalanceMinor ?? req.user.walletBalanceMinor, idempotent: true });
      }
    }
    let post;
    let charge = 0;
    ...
    await session.withTransaction(async () => {
      ...
      const created = await Post.create([{ ...normalized, ...(clientSubmissionId ? { clientSubmissionId } : {}), moderationStatus: 'approved', author: req.user.displayName || req.user.username, ownerId: req.user._id, ownerUsername: req.user.username, updatedAt: new Date() }], { session });
      post = created[0];
    });
...
app.get(['/return-policy', '/returns', '/refund-policy'], (req, res) => res.type('html').set('Cache-Control', 'public, max-age=3600, s-maxage=86400').send(returnPolicyHtml()));
app.get('/share-card.png', async (req, res) => { try { const title = clean(req.query.title || 'Lee Tech', 90); const subtitle = clean(req.query.subtitle || 'Technology with intention.', 170); const kicker = clean(req.query.kicker || 'LEE TECH COMMUNITY', 42); const url = clean(req.query.url || 'POST.LEETEC.ONLINE', 64); const type = clean(req.query.type || 'CREATOR SITE', 24); const image = await renderShareCard({ title, subtitle, kicker, url, type }); res.type('png').set('Cache-Control', 'public, max-age=300, s-maxage=3600, stale-while-revalidate=86400').send(image); } catch (error) { console.error('Share-card generation error:', error.message); res.status(500).end(); } });
app.get('/lee-tech-hero.webp', (req, res) => { if (!heroImage) return res.status(404).end(); res.type('image/webp').set('Cache-Control', 'public, max-age=31536000, immutable').send(heroImage); });
...
function returnPolicyHtml() {
  return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Returns & refunds — Lee Tech</title><meta name="description" content="Lee Tech returns, refunds, cancellations, and digital product policy."><meta name="robots" content="index,follow"><style>body{margin:0;background:#f7f8fb;color:#12233f;font:16px system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;line-height:1.7}.wrap{width:min(820px,calc(100% - 40px));margin:0 auto;padding:48px 0 72px}.brand{display:inline-flex;align-items:center;gap:10px;color:#12233f;text-decoration:none;font-weight:800;font-size:20px}.mark{display:grid;place-items:center;width:38px;height:38px;border-radius:12px;background:#0e1b32;color:#bdebdc}.card{margin-top:32px;background:#fff;border:1px solid #e5eaf1;border-radius:26px;padding:clamp(26px,5vw,58px);box-shadow:0 24px 70px rgba(22,43,78,.08)}.eyebrow{color:#1f5eff;font-size:12px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.card h1{font-size:clamp(36px,7vw,62px);letter-spacing:-.06em;line-height:1.05;margin:16px 0}.lead{color:#6d7b92;font-size:18px}.card h2{margin-top:34px;font-size:24px}.card a{color:#1f5eff;font-weight:700}.notice{padding:16px 18px;border-left:4px solid #1f5eff;background:#eef3ff;border-radius:10px}.back{display:inline-block;margin-top:28px;color:#1f5eff;font-weight:700}</style></head><body><main class="wrap"><a class="brand" href="/"><span class="mark">L</span> Lee Tech</a><article class="card"><div class="eyebrow">Customer policy</div><h1>Returns & refunds</h1><p class="lead">We want every Lee Tech purchase to be clear, useful, and handled fairly. This page explains how to request help with a product, digital item, or paid service.</p><p><strong>Last updated:</strong> September 7, 2026</p><div class="notice"><strong>Need help?</strong> Contact us as soon as possible at <a href="mailto:marketpro.online.admin@gmail.com">marketpro.online.admin@gmail.com</a>, <a href="tel:+254116553618">+254 116 553 618</a>, or <a href="https://wa.me/254116553618">WhatsApp support</a> with your order or payment reference, the item or service name, and the reason for your request.</div><h2>Physical products</h2><p>Contact us promptly if a physical product arrives damaged, defective, materially different from its description, or incomplete. Include clear photos where relevant. We will review the request and, where approved, arrange an appropriate replacement, repair, or refund. Return instructions must be received from Lee Tech before sending an item back.</p><h2>Digital products</h2><p>Because digital products may be accessed or delivered immediately, refunds are generally considered when the file is defective, inaccessible, materially different from its description, or duplicated by a technical error. Please contact us before sharing or distributing the digital item so we can investigate and help resolve the issue.</p><h2>Paid services</h2><p>For a paid service, contact us before substantial work begins if you need to cancel. Once work has started, any refund is assessed against the work already completed and any third-party costs already incurred.</p><h2>Payment reversals and duplicate charges</h2><p>If a payment was duplicated, charged incorrectly, or completed but the wallet or order was not updated, send the payment reference to us. We will investigate the transaction and correct confirmed errors. A payment being pending or declined does not by itself mean that a refund is due; the payment status will be checked with the payment provider.</p><h2>How approved refunds are issued</h2><p>Approved refunds are returned through the original payment method where possible. Processing time depends on the payment provider and financial institution. We will confirm the decision and any next steps by email.</p><h2>Contact</h2><p>For return, refund, cancellation, or payment questions, email <a href="mailto:marketpro.online.admin@gmail.com">marketpro.online.admin@gmail.com</a>. Please do not send passwords or full card details.</p><a class="back" href="/">← Back to Lee Tech</a></article></main></body></html>`;
}
