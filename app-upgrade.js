(() => {
  ...
  function renderUserLibrarySections(posts = []) {
    ...
    const rows = (items, empty) => items.length ? items.map(post => `<div class="upgrade-history-row"><div><strong>${escapeHtml(post.title)}</strong><small>${post.published ? 'Published on your site' : 'Draft'} · ${formatUserDate(post.createdAt)}</small></div><button class="ghost" data-upgrade-edit-post="${escapeHtml(post._id)}">Edit</button></div>`).join('') : `<div class="upgrade-empty-state">${empty}</div>`;
  ...
  async function beginTopup(event) {
    ...
    try { const result = await request('/api/wallet/paystack/initialize', { method: 'POST', body: JSON.stringify({ amount }) }); if (!result?.authorizationUrl || !result.reference) throw new Error('Paystack did not return a valid checkout session'); notify('Opening secure Paystack checkout…', 'info'); window.location.assign(result.authorizationUrl); }
    catch (error) { notify(error.message, 'error'); }
  }
  async function saveUserPost(event) {
    ...
    const body = { ...values, contentType, productType: contentType === 'product' ? values.productType : 'digital', price: contentType === 'product' ? values.price : '', stock: contentType === 'product' && values.productType === 'physical' ? values.stock : '', image: pendingUserPostImage || values.image || '', published: values.published === 'true' };
    const message = form.querySelector('#upgradePostMessage');
    try { await request('/api/me/posts' + (userState.post ? `/${userState.post._id}` : ''), { method: userState.post ? 'PUT' : 'POST', body: JSON.stringify(body) }); userState.post = null; message.textContent = contentType === 'product' ? 'Product saved successfully.' : 'Blog saved successfully.'; notify(contentType === 'product' ? 'Product saved successfully.' : 'Blog saved successfully.', 'success'); pendingUserPostImage = ''; form.reset(); renderUserPostImagePreview(); updateUserPostTypeFields(); await loadDashboard(); }
    catch (error) { message.textContent = error.message; notify(error.message, 'error'); }
  }
  ...
  async function saveUserProfile(event) {
    ...
    try {
      const values = Object.fromEntries(new FormData(form).entries());
      const contactFields = ['whatsappNumber', 'whatsappGroupLink', 'instagramUrl', 'facebookUrl', 'xUrl', 'linkedinUrl', 'tiktokUrl', 'youtubeUrl', 'telegramUrl', 'websiteUrl'];
  ...
  async function handlePaymentCallback() {
    ...
    window.history.replaceState({}, '', window.location.pathname);
    if (['failed', 'abandoned', 'cancelled', 'canceled'].includes(status)) { notify('Payment was not completed. No wallet funds were added.', 'error'); if (userState.user) openDashboard(); return; }
    if (!reference) { notify('Paystack returned without a payment reference. Check your Paystack callback configuration.', 'error'); if (userState.user) openDashboard(); return; }
    try { const data = await request(`/api/wallet/paystack/verify/${encodeURIComponent(reference)}`, { method: 'POST' }); notify(`Wallet credited: ${money(data.balanceMinor)}`, 'success'); if (userState.user) openDashboard(); }
    catch (error) { notify(`Payment could not be confirmed yet: ${error.message}. Use Check status in Wallet & history.`, 'info'); if (userState.user) openDashboard(); }
  }
  ...
  function renderPublicProductCards(products = [], contact = {}) { if (!products.length) return '<div class="upgrade-public-empty"><div class="upgrade-public-empty-icon">◈</div><h3>No products published yet.</h3><p>Digital and physical products from this creator will appear here when published.</p></div>'; const digits = String(contact.whatsappNumber || '').replace(/\D/g, ''); return products.map((product, index) => { const physical = product.productType === 'physical'; const stockLabel = physical ? (product.stock === 0 ? 'Out of stock' : `${product.stock == null ? 'Available' : product.stock} in stock`) : 'Digital delivery'; const buyable = digits.length >= 7 && digits.length <= 15 && (!physical || product.stock !== 0); const buyHref = buyable ? `https://wa.me/${digits}?text=${encodeURIComponent(`Hello, I would like to buy ${product.title}.`)}` : ''; const action = buyable ? `<a class="primary upgrade-product-buy" href="${escapeHtml(buyHref)}" target="_blank" rel="noopener noreferrer">Buy via WhatsApp ↗</a>` : `<button class="ghost upgrade-product-share" type="button" data-share-product data-share-title="${escapeHtml(product.title)}" data-share-text="${escapeHtml(product.excerpt || product.content.slice(0,170))}">Share product ↗</button>`; return `<article class="upgrade-public-product"><div class="upgrade-public-product-topline"><span>${physical ? 'Physical product' : 'Digital product'}</span><b>${index + 1 < 10 ? '0' : ''}${index + 1}</b></div>${product.image ? `<img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.title)}" class="upgrade-public-product-image" loading="lazy" decoding="async">` : '<div class="upgrade-public-product-placeholder">◈</div>'}<div class="upgrade-public-product-kicker">${escapeHtml(product.author || 'Creator product')}</div><h3>${escapeHtml(product.title)}</h3><p>${escapeHtml(product.content)}</p><div class="upgrade-public-product-foot"><strong>${money(product.priceMinor || 0)}</strong><span>${escapeHtml(stockLabel)}</span></div>${action}</article>`; }).join(''); }
  ...
  async function renderPublicUserSite(silent = false) {
    try {
      ...
      site.querySelectorAll('[data-share-post], [data-share-product]').forEach(button => button.addEventListener('click', () => window.shareItem?.(button.dataset.shareTitle, button.dataset.shareText, '', `${location.origin}${location.pathname}`)));
    } catch {
