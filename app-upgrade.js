(() => {
  ...
  async function beginTopup(event) {
    ...
    try { const result = await request('/api/wallet/paystack/initialize', { method: 'POST', body: JSON.stringify({ amount }) }); if (!result?.authorizationUrl || !result.reference) throw new Error('Paystack did not return a valid checkout session'); window.location.assign(result.authorizationUrl); }
    catch (error) { notify(error.message, 'error'); }
  }
  async function saveUserPost(event) {
    ...
    try { await request('/api/me/posts' + (userState.post ? `/${userState.post._id}` : ''), { method: userState.post ? 'PUT' : 'POST', body: JSON.stringify(body) }); userState.post = null; message.textContent = contentType === 'product' ? 'Product saved successfully.' : 'Blog saved successfully.'; pendingUserPostImage = ''; form.reset(); renderUserPostImagePreview(); updateUserPostTypeFields(); await loadDashboard(); }
    catch (error) { message.textContent = error.message; }
  }
