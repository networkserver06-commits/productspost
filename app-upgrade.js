(() => {
  ...
  async function beginTopup(event) {
    ...
    if (!Number.isFinite(numericAmount) || numericAmount < 4) return notify('Minimum deposit is KES 4.00.', 'error');
    const submit = event.submitter;
    if (submit) submit.disabled = true;
    try {
      const result = await request('/api/wallet/paystack/initialize', { method: 'POST', body: JSON.stringify({ amount }) });
