(() => {
  ...
  function updateUserPostCostPreview() { const form = document.getElementById('upgradePostForm'); if (!form) return; const cost = Number(userState.postingCostMinor || 0); const balance = Number(userState.wallet?.balanceMinor || 0); const isEdit = !!userState.post; const wasPublished = !!userState.post?.published; const isPublishing = form.elements.published?.checked === true; const willCharge = isPublishing && !wasPublished; const charge = willCharge ? cost : 0; const costNode = document.getElementById('upgradePostCost'); const detailNode = document.getElementById('upgradePostCostDetail'); const balanceNode = document.getElementById('upgradePostBalance'); if (costNode) costNode.textContent = userState.postingCostMinor === null ? 'Checking current price…' : charge ? money(charge, userState.postingCostCurrency) : 'No deduction'; if (detailNode) detailNode.textContent = isEdit && wasPublished ? 'Editing this published item does not charge a new publishing fee.' : willCharge ? (balance >= charge ? 'This exact amount will be deducted only after you confirm.' : 'Top up before publishing to cover this publishing fee.') : 'Save as a draft at no cost, or turn on Publish now to review the exact deduction.'; if (balanceNode) balanceNode.textContent = money(Math.max(0, balance - charge), userState.postingCostCurrency); const receipt = document.getElementById('upgradePostReceipt'); if (receipt) receipt.innerHTML = userState.postReceipt ? `<strong>Latest receipt: ${userState.postReceipt.chargedMinor ? 'Deducted ' + escapeHtml(money(userState.postReceipt.chargedMinor, userState.postReceipt.currency)) : 'No publishing fee deducted.'}</strong><span>Remaining wallet balance: ${escapeHtml(money(userState.postReceipt.balanceAfterMinor, userState.postReceipt.currency))}</span>` : ''; const review = document.getElementById('upgradePostReview'); if (review) { review.hidden = !isPublishing; review.textContent = isPublishing ? (willCharge ? `Review: ${money(charge, userState.postingCostCurrency)} will be deducted. Estimated remaining balance: ${money(Math.max(0, balance - charge), userState.postingCostCurrency)}.` : 'Review: no additional publishing fee will be deducted for this edit.') : ''; } }
  ...
  async function saveUserPost(event) {
    ...
    const fee = isPublishing && !wasPublished ? Number(userState.postingCostMinor || 0) : 0;
    ...
    const review = isPublishing ? (fee ? `Review publishing\n\n${money(fee, userState.postingCostCurrency)} will be deducted from your wallet.\nEstimated remaining balance: ${money(balance - fee, userState.postingCostCurrency)}.\n\nContinue only if the title, details, product price, and stock are correct.` : 'Review this edit\n\nNo additional publishing fee will be deducted.\n\nContinue only if all details are correct.') : `Review saving this ${contentType === 'product' ? 'product' : 'blog'} as a draft.\n\nNo wallet deduction will be made.`;
